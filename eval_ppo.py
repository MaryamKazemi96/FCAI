...
def _is_rew_key(k: Any) -> bool:
    # keep all rew/* numeric keys except reward_mode marker
    return isinstance(k, str) and k.startswith("rew/") and k not in {"rew/reward_mode"}
...
def run_evaluation(
    ...
    # Dynamic reward component keys discovered from info["rew/*"]
    ...
    for ep in range(n_episodes):
        ...
        # per-episode running sums for any rew/* keys encountered
        ep_comp: Dict[str, float] = {}
        ...
        while not done:
            ...
            info = infos[0] if isinstance(infos, (list, tuple)) else infos
            if isinstance(info, dict):
                # accumulate any numeric rew/* keys
                for k, v in info.items():
                    if not _is_rew_key(k):
                        continue
                    if isinstance(v, (int, float, np.number)):
                        ep_comp[k] = ep_comp.get(k, 0.0) + float(v)
        ...
        # append episode sums for whatever keys appeared
        for k, s in ep_comp.items():
            comp_sums.setdefault(k, []).append(float(s))

        # pad missing keys for this episode with 0.0 to keep arrays same length
        for k, arr in comp_sums.items():
            if len(arr) < len(rewards):
                arr.append(0.0)
