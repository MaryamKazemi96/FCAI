# """
# SB3 policy wrapper (colleague-style):
...
# - computes logprob/entropy over active robots only
# """
...
#     This extractor just returns zeros; the policy ignores it.
#     """
...
#         return th.zeros((B, self._features_dim), device=observations["node_features"].device)
...
#         self.K = self.Kp1 - 1  # candidates only, without NOOP
#         assert self.K == int(k_max), f"k_max mismatch: action space implies K={self.K}, got k_max={k_max}"
#         self.noop_index = self.K  # last column after append
...
#         # IMPORTANT: SB3 would normally use self.action_net/value_net; we bypass them
#         # but they must exist. We keep dummy heads.
...
#         """
#         logits_k: [B,R,K]
#         returns:  [B,R,K+1] with NOOP logit = 0 (shared)
#         """
#         B, R, K = logits_k.shape
#         noop = th.zeros((B, R, 1), device=logits_k.device, dtype=logits_k.dtype)
#         return th.cat([logits_k, noop], dim=-1)
...
#     #     logits: [B,R,Kp1]
#     #     actions: [B,R]
#     #     active: [B,R] -> include only active robots in sums
#     #     """
...
#     #     return chosen_logp.sum(dim=1), ent.sum(dim=1)        # [B], [B]
#     @staticmethod
#     def masked_logprob_entropy(logits: th.Tensor, actions: th.Tensor) -> Tuple[th.Tensor, th.Tensor]:
#         """
...
#         mask_full = obs_dict["action_mask"]         # [B,R,K+1] (0/1)
...
#         logits[..., -1] = th.where(mask_full[..., -1], logits[..., -1], th.zeros_like(logits[..., -1]))

#         dist = self._dist_from_logits(logits)
#         actions = dist.get_actions(deterministic=deterministic)  # [B,R]

#         # active robots = those with at least one valid candidate (excluding NOOP)
#         # active = mask_full[..., :-1].any(dim=-1)                 # [B,R]
#         # active = mask_full.any(dim=-1)  # includes NOOP column
#         # log_prob, _ = self.masked_logprob_entropy(logits, actions, active)
#         log_prob, _ = self.masked_logprob_entropy(logits, actions)
...
#         logits_k, values = self.model(x, edge_index, num_nodes, num_edges, cand_node_idx)
#         logits = self._append_noop(logits_k)
...
#         log_prob, entropy = self.masked_logprob_entropy(logits, actions)
#         return values, log_prob, entropy

#     def predict_values(self, obs: Any) -> th.Tensor:
#         obs_dict = cast(Dict[str, th.Tensor], obs)
#         logits_k, values = self.model(
#             obs_dict["node_features"],
#             obs_dict["edge_index"],
#             obs_dict["num_nodes"],
#             obs_dict["num_edges"],
#             obs_dict["cand_node_idx"],
#         )
#         return values
...
class RTGNNPolicy(ActorCriticPolicy):
    def __init__(
        ...
        activation: str = "relu",
        dropout: float = 0.0,
        # --- new knobs (colleague-style) ---
        noop_init: float = -1.0,
        noop_frozen: bool = True,
